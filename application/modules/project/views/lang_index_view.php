...
<div class="container">
  ...
  <div class="card-columns">
    ...
    foreach ($project as $key => $value) { ?>
    	<div class="card <?php if($value->status=='1'){ ?>bg-warning<?php }else { ?>bg-warning <?php }?>">
	      <div class="card-body text-center">
	        <p class="card-text"><?php echo $value->project_name; ?></p>
	        <p><small>
            <?php if ($uscount[$key][0]==''){$uscount[$key][0]=1; }$percentage=(($uscount[$key][0]-$count[$key][0])/$uscount[$key][0])*100; ?>
            <?=$count[$key][0]; ?>/<?=$uscount[$key][0]; ?> Strings remaining</small></p>
            <a href="<?=base_url();?>contribute/project/<?=$value->project_name?>"><button class="btn btn-success"><?=round($percentage,2)?>% Completed</button> </a>
	      </div>
	    </div>
